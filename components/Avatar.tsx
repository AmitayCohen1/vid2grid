...
/* ---------- one download per file, with progress anyone can watch ---------- */

export interface AvatarLoad { url: string; done: number; total: number }
const files = new Map<string, Promise<ArrayBuffer>>();
const loads = new Map<string, AvatarLoad>();
let snapshot: AvatarLoad[] = [];
const listeners = new Set<() => void>();
const notify = () => { snapshot = [...loads.values()]; for (const l of listeners) l(); };

/** Fetch a character file once, however many dancers wear it, reporting bytes as they arrive. */
function fetchModel(file: string): Promise<ArrayBuffer> {
  const cached = files.get(file);
  if (cached) return cached;
  loads.set(file, { url: file, done: 0, total: 0 }); notify();
  const p = (async () => {
    const res = await fetch(file);
    if (!res.ok) throw new Error(`character download failed: ${res.status}`);
    const total = Number(res.headers.get("content-length")) || 0;
    loads.set(file, { url: file, done: 0, total }); notify();
    ...
    for (;;) {
      ...
      if (performance.now() - last > 80) { last = performance.now(); loads.set(file, { url: file, done, total }); notify(); }
    }
  ...
  p.finally(() => { loads.delete(file); notify(); }).catch(() => {});
  p.catch(() => files.delete(file)); // a failed download can be retried
  files.set(file, p);
  return p;
}
...
/** Characters being downloaded right now, for a progress note on the stage. */
export function useAvatarLoading(): AvatarLoad[] {
...
/** A preset's look: multiply a colour into every MToon material whose name
 *  contains the key (VRoid names them by part — Tops, Bottoms, Hair…).
 *  Each load parses its own materials, so this never touches another rig. */
function tintMaterials(scene: THREE.Object3D, tint: Record<string, string> | undefined) {
  if (!tint) return;
  ...
  scene.traverse((o) => {
    ...
    for (const m of Array.isArray(mats) ? mats : mats ? [mats] : []) {
      ...
      seen.add(m);
      for (const [key, hex] of Object.entries(tint)) {
        if (!m.name.includes(key)) continue;
        const c = new THREE.Color(hex);
        const mt = m as THREE.Material & { color?: THREE.Color; shadeColorFactor?: THREE.Color };
        mt.color?.multiply(c);
        mt.shadeColorFactor?.multiply(c);
      }
    }
  });
}

/** A VRM scene can only appear once in the graph, so each on-stage
 *  instance (`key`) parses its own rig from the one shared download. */
function loadRig(url: string, key = ""): Promise<Rig> {
  ...
  const p = (async () => {
    ...
    const gltf = await loader.parseAsync(await fetchModel(avatarFile(url)), "");
    const vrm = gltf.userData.vrm as VRM;
    VRMUtils.removeUnnecessaryVertices(gltf.scene);
    VRMUtils.combineSkeletons(gltf.scene);
    VRMUtils.rotateVRM0(vrm); // no-op for VRM 1.0; makes 0.x face +z like 1.0
    vrm.scene.traverse((o) => { o.frustumCulled = false; });
    tintMaterials(vrm.scene, avatarTint(url));
  ...
  rigCache.set(cacheKey, p);
  p.catch(() => rigCache.delete(cacheKey)); // let a failed load be retried
  return p;
}
...
function VrmAvatar({ pose, body, url, instanceKey }: { pose: Pose; body: Body; url: string; instanceKey: string }) {
  ...
  useEffect(() => {
    let alive = true;
    loadRig(url, instanceKey).then((rig) => { if (alive) setLoaded({ url, rig }); }).catch((e) => console.error("avatar load failed", e));
    return () => { alive = false; };
  }, [url, instanceKey]);
  ...
  // Until the character is here (or while a new one is on its way), the
  // stick figure dances in its place: the movement never stops.
  if (!rig) return <Figure pose={pose} body={body} />;
