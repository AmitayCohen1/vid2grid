...
export default function Source({ onFile, busy }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="seg self-start text-xs">
        <button aria-pressed={mode === "upload"} onClick={() => setMode("upload")}>upload</button>
        <button aria-pressed={mode === "record"} onClick={() => setMode("record")}>record</button>
      </div>
...
function Upload({ onFile, busy }: Props) {
  ...
  return (
    <label
      ...
      className={`block cursor-pointer rounded-lg border-2 border-dashed p-8 text-center text-sm transition-colors ${over ? "border-ring bg-accent" : "bg-card hover:border-muted-foreground/50"}`}
    >
      <div className="text-foreground font-medium">Drop a short video here, or click to choose</div>
      <div className="text-muted-foreground text-xs mt-1">5–30 s works best · one dancer, full body in frame, camera still</div>
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2 text-xs">
        {!recording ? (
          <button className="btn primary" disabled={!ready || busy} onClick={start}>● record</button>
        ) : (
          <button className="btn" onClick={stop}>■ stop &amp; analyse</button>
        )}
        <span className="text-muted-foreground">Step back so your whole body is in frame. Stop when done; the clip is analysed right away.</span>
      </div>
    </div>
  );
}
