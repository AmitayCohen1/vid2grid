  ...
  return (
    <section className="flex flex-col gap-2.5 p-4 pt-0 text-[13px]">
      <button className="btn self-start" disabled={!canAdd} onClick={onAdd} title="pin the current dancer (clip + look) onto the stage">
        <Plus size={18} /> Add this dancer to the cast
      </button>
      {cast.length === 0 ? (
            ...
            <div key={m.id} className="card p-2.5 flex flex-col gap-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="ml-auto flex items-center gap-1">
                  <button className="icon-button" aria-label={`Duplicate ${m.name}`} title="Duplicate dancer" onClick={() => onDuplicate(m.id)}><Copy size={18} /></button>
                  <button className="icon-button" aria-label={`Remove ${m.name}`} title="Remove from cast" onClick={() => onRemove(m.id)}><X size={18} /></button>
                </span>
              </div>
              <Slider label={`Left / right: ${m.x.toFixed(1)} m`} min={-4} max={4} step={0.1} value={m.x} onChange={(x) => onUpdate(m.id, { x })} />
              <Slider label={`Front / back: ${m.z.toFixed(1)} m`} min={-4} max={4} step={0.1} value={m.z} onChange={(z) => onUpdate(m.id, { z })} />
              <Slider label={`Facing: ${Math.round(m.rot)}°`} min={-180} max={180} step={5} value={m.rot} onChange={(rot) => onUpdate(m.id, { rot })} />
            </div>
          ))}
        </div>
      )}
