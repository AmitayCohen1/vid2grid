...
import { ArrowUpRight, Camera, FileUp, Play, Radio, ShieldCheck, UploadCloud } from "lucide-react";
...
export default function NewScore({ onFile, onImport, onDemo, onLive, busy, progress, loading, onCancel, initialMode = "upload" }: {
  ...
  return <>
    ...
    {mode === "live" && <div className="import-score-flow"><p>Dance in front of the camera and watch the dance form as you move — the figure, the grid, and your cast, all in real time. When you finish, the take becomes a dance like any other, video included.</p><button className="upload-zone live-zone" onClick={onLive}><Radio size={30} /><strong>Go live</strong><span>Whole body in frame, still camera · up to 2 minutes</span></button></div>}
    {mode === "import" && <div className="import-score-flow"><p>Pick a previously downloaded vid2grid dance to continue exploring it. The original video is not included in a dance file.</p><label className="upload-zone"><FileUp size={30} /><strong>Open a saved dance</strong><span>Choose a .vid2grid.json file · up to 25 MB</span><input aria-label="Open saved dance" type="file" accept=".json,application/json" className="sr-only" onChange={(e) => { const f = e.target.files?.[0]; if (f) onImport(f); e.currentTarget.value = ""; }} /></label></div>}
