...
export default function NewScore({ onFile, onImport, onDemo, onLive, busy, progress, loading, onCancel, initialMode = "upload" }: {
  ...
  return <>
    ...
    {mode === "live" && <div className="import-score-flow"><p>Dance in front of the camera and watch the score form as you move — the figure, the grid, and your cast, all in real time. When you finish, the take becomes a score like any other, video included.</p><button className="upload-zone live-zone" onClick={onLive}><Radio size={30} /><strong>Go live</strong><span>Whole body in frame, still camera · up to 2 minutes</span></button></div>}
