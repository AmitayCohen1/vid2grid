...
- Sections/choreography: `lib/sectionize.ts` + `lib/sectionsDb.ts` have a deliberately duplicated vanilla twin in `public/movement-languages/section-store.js` (static pages can't import from the app) — keep the schema in step. The static pages under `public/movement-languages/` are the app edition and the ones to edit. `../standalone/` holds a separate, deliberately diverged edition of the same two pages — no-build, opened straight from disk, with Claude artifact URLs hard-coded for cross-linking when hosted. It is not a copy to keep in sync, and is not touched from here. `npm test` runs node:test units; `npm run e2e` needs the dev server.
