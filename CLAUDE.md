...
- Sections/choreography: `lib/sectionize.ts` + `lib/sectionsDb.ts` have a deliberately duplicated vanilla twin in `public/movement-languages/section-store.js` (static pages can't import from the app) — keep the schema in step. `public/movement-languages/notation-render.js` is the shared pose kernel — bone model, `skeleton()` FK, limb helpers, `poseAt()` interpolation and the Laban quantizer live there once; `danceforms.html` loads it and rebinds them in a destructuring preamble, and `comparison.html` calls them directly. Add a kernel symbol in all three places: the module's export list, `KERNEL_EXPORTS` in `test/notation.test.mjs`, and `danceforms.html`'s destructuring preamble. The three notation *renderers* stay duplicated on purpose (Studio's carry selection and editing tools); `danceforms.html` is no longer frozen. The static pages under `public/movement-languages/` are the app edition and the ones to edit. `../standalone/` holds a separate, deliberately diverged edition of the same two pages — no-build, opened straight from disk, with Claude artifact URLs hard-coded for cross-linking when hosted. It is not a copy to keep in sync, and is not touched from here. `npm test` runs node:test units; `npm run e2e` needs the dev server.

## Comparison explainer — "one movement, four readings" (invariants)

The `public/movement-languages/comparison.html` explainer and its `notation.js` model (twin-tested via `test/notation.test.mjs`) obey these eight invariants. They come from `../docs/movement-notation-parameters.md` §7 and `../docs/movement-notation-visualization2.md` §7. Changing any of them is a design decision, not a refactor — stop and confirm.
