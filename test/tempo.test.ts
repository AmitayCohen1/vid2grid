...
test("a single slow swell is not a tempo", () => {
  // One cycle over the whole clip (~7.5 bpm, far under the 30 bpm floor).
  // Autocorrelation is near 1 at every short lag here, so a search that just
  // maximises it reports the top of the range at high confidence. Only an
  // interior, prominent peak counts as a pulse.
  const raw = synthRaw(8, 30, 8);
  const est = estimateTempo(raw, [], 30);
  assert.equal(est.confidence, 0, `claimed ${est.bpm} bpm on an unpulsed clip`);
